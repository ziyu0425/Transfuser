...
class TransfuserBackbone(nn.Module):
    ...
    """
    def __init__(self, image_architecture='regnety_032', lidar_architecture='regnety_032', use_velocity=False,use_target_point_image=True,out_feat=512): # here I just use resnet 34 and resnet18
        super().__init__()
        self.img_vert_anchors = 5
        self.img_horz_anchors = 20 + 2
        ...
        self.avgpool_lidar = nn.AdaptiveAvgPool2d((self.lidar_vert_anchors, self.lidar_horz_anchors))
        self.image_encoder = ImageCNN(architecture=image_architecture, normalize=True,
                                      out_features=out_feat)
        self.in_channels = 2
        self.perception_output_features = 512
        ...
        self.lidar_encoder = LidarEncoder(architecture=lidar_architecture, in_channels=self.in_channels,
                                          out_features=out_feat)
        self.transformer1 = GPT(n_embd=self.image_encoder.features.feature_info[1]['num_chs'],
                            ...
                            lidar_vert_anchors=self.lidar_vert_anchors,
                            lidar_horz_anchors=self.lidar_horz_anchors,
        ...
        if(self.image_encoder.features.feature_info[4]['num_chs'] != self.perception_output_features):
            self.change_channel_conv_image = nn.Conv2d(self.image_encoder.features.feature_info[4]['num_chs'], self.perception_output_features, (1, 1))
            self.change_channel_conv_lidar = nn.Conv2d(self.image_encoder.features.feature_info[4]['num_chs'], self.perception_output_features, (1, 1))
        else:
            self.change_channel_conv_image = nn.Sequential()
            self.change_channel_conv_lidar = nn.Sequential()

        # FPN fusion
        channel = self.bev_features_chanels
        self.relu = nn.ReLU(inplace=True)
        # top down
        self.upsample = nn.Upsample(scale_factor=self.bev_upsample_factor, mode='bilinear', align_corners=False)
        ...
        self.c5_conv = nn.Conv2d(self.perception_output_features, channel, (1, 1))
    def top_down(self, x):
    ...
    def forward(self, image, lidar, target_point=None, velocity=None):
      '''
      Image+LiDAR feature fusion using transformers
      '''
      if self.image_encoder.normalize:
        image_tensor = self.image_encoder.normalize_imagenet(image)
      else:
        image_tensor = image
      lidar_tensor = lidar
      ...
      #torch.Size([1, 64, 225, 1050]) torch.Size([1, 64, 250, 250])
      ...
      # (B,64,225,1050) image, (B,64,250,250) bev
      
      image_features = self.image_encoder.features.layer2(image_features)
      lidar_features = self.lidar_encoder._model.layer2(lidar_features)
      # torch.Size([B, 128, 113, 525]) torch.Size([B, 128, 125, 125])
      ...
      # torch.Size([B, 128, 5, 22]) torch.Size([B, 128, 8, 8])
      ...
      # 3 torch.Size([1, 256, 57, 263]) torch.Size([1, 256, 63, 63])
      ...
      #4 torch.Size([1, 512, 29, 132]) torch.Size([1, 512, 32, 32])
      ...
      #ds torch.Size([1, 512, 29, 132]) torch.Size([1, 512, 32, 32])
      ...
      #global pool:  torch.Size([1, 512])
      #flatten:  torch.Size([1, 512])
      #lidar global pool:  torch.Size([1, 512])
      #lidar flatten:  torch.Size([1, 512])

      fused_features = image_features + lidar_features
      features = self.top_down(x4)
      #top_down:  torch.Size([1, 64, 256, 256]) torch.Size([1, 64, 128, 128]) torch.Size([1, 64, 64, 64]) torch.Size([1, 64, 32, 32])
...
class GPT(nn.Module):
    ...
    def __init__(self, n_embd, n_head, block_exp, n_layer, 
                    ...
                    seq_len, 
                    embd_pdrop, attn_pdrop, resid_pdrop,  use_velocity=False):
        super().__init__()
        self.n_embd = n_embd
